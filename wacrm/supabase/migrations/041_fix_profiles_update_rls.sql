-- ============================================================
-- 041_fix_profiles_update_rls.sql — lock down privilege columns
--                                    on profiles (GHSA-fg5p-2qc3-jmxr, C1)
--
-- NOTE: re-sequenced 034 → 041 to sidestep a duplicate migration version.
-- The 034 slot was taken by 034_ai_providers_expand.sql, so shipping this
-- as 034 too made a clean `supabase db` apply fail with a duplicate
-- schema_migrations key (SQLSTATE 23505). This migration is idempotent
-- (DROP TRIGGER IF EXISTS / CREATE OR REPLACE) and independent of the AI
-- tables, so re-sequencing it after 040 is safe.
--
-- The problem
--
