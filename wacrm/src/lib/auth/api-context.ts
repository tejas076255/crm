...
import { planFromValue, type Plan } from './plans';
...
export interface ApiKeyContext {
  ...
  /** The account's current plan — for plan-gating API surfaces. */
  plan: Plan;
}
...
export async function requireApiKey(
  ...
  // Resolve the account's plan so API surfaces can be plan-gated
  // (automations/flows = pro, AI = pro_max). Defaults to 'free' on any
  // lookup miss/error — a public-API caller should never be granted
  // more than the strictest tier by accident.
  ...
  const { data: accountPlan } = await admin
    ...
    .maybeSingle();
  const plan = planFromValue(accountPlan?.plan);
