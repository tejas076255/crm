...
import { hasMinPlan, planFromValue, type Plan } from "./plans";
...
export interface AccountContext {
  ...
  role: AccountRole;
  /** Caller's account plan ('free' | 'pro' | 'pro_max'). */
  plan: Plan;
...
/**
 * Resolve the caller's account context and enforce a minimum plan.
 *
 * Throws `UnauthorizedError` / `ForbiddenError` as documented on
 * `getCurrentAccount`, plus `ForbiddenError("plan required")` when the
 * caller's account is below `min`. Used to gate feature surfaces
 * (Automations & Flows = 'pro', AI = 'pro_max').
 */
export async function requirePlan(min: Plan): Promise<AccountContext> {
  const ctx = await getCurrentAccount();
  if (!hasMinPlan(ctx.plan, min)) {
    throw new ForbiddenError(`This feature requires the '${min}' plan or higher`);
  }
  return ctx;
}
