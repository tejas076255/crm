...
import {
  canAutomate as canAutomateFor,
  canUseAI as canUseAIFor,
  planFromValue,
  planStatusFromValue,
  type Plan,
  type PlanStatus,
...
interface AccountSummary {
  ...
  /** Account plan ('free' | 'pro' | 'pro_max') — migration 040. */
  plan: Plan;
  /** Account plan_status ('none' | 'active' | 'past_due' | 'cancelled').
   *  Nullable account rows narrow to 'none'. */
...
interface AuthContextValue {
  ...
  /** The account's plan ('free' | 'pro' | 'pro_max'). Defaults to
   *  'free' while loading or when no account is resolved, so UI gates
   *  fail closed. */
  plan: Plan;
  /** The account's subscription health. Defaults to 'none' while
   *  loading. The Billing panel uses it to show Active/Cancelled state;
   *  capability boolean gates still key off `plan` alone. */
  planStatus: PlanStatus;
  /** True if the account has the automations & flows surface (pro+). */
  canAutomate: boolean;
  /** True if the account has the AI surface (pro_max only). */
  canUseAI: boolean;
...
export function AuthProvider({ children }: { children: ReactNode }) {
      ...
      if (data) {
        ...
        if (data.account_id) {
          const { data: account, error: accountErr } = await supabase
            .from("accounts")
            // default_currency added in migration 021; plan in 040.
            // default_currency falls back to USD for older schemas;
            // plan narrows to 'free' via planFromValue; plan_status
            // narrows to 'none' via planStatusFromValue (same migration).
            .select("id, name, default_currency, plan, plan_status")
          ...
          if (accountErr) {
            ...
            accountRow = {
              ...
              plan: planFromValue(account.plan),
              plan_status: planStatusFromValue(account.plan_status),
            };
          }
        }
  ...
  const derived = useMemo(() => {
    ...
    return {
      planStatus: account?.plan_status ?? "none",
      accountRole: role,
      ...
      plan,
      canAutomate: canAutomateFor(plan),
      canUseAI: canUseAIFor(plan),
    };
  }, [profile?.account_role, profile?.account_id, account?.plan, account?.plan_status]);
