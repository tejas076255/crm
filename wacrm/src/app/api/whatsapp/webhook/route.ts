...
export async function POST(request: Request) {
  ...
  // Fetch candidate secrets stored in database (encrypted)
    ...
    if (configs) {
      for (const c of configs) {
        if (c.meta_app_secret) {
          try {
            const dec = decrypt(c.meta_app_secret)
            if (dec) candidateSecrets.push(dec)
          } catch {
            // ignore decryption error if key changed
          }
        }
      }
    }
  } catch (err) {
    console.warn('[webhook] Could not fetch DB meta_app_secret candidates:', err)
  }
