...
import { AiError, isValidAiProvider, type AiProvider } from '@/lib/ai/types'
import { encodeDbModel, decodeDbModel } from '@/lib/ai/config'
...
export async function GET() {
  try {
    // AI surface is a pro_max plan feature — deny lower plans.
    ...
    const { api_key, embeddings_api_key, provider: dbProvider, model: dbModel, ...safe } = data
    const decoded = decodeDbModel(dbProvider, dbModel)
    return NextResponse.json({
      configured: true,
      has_key: !!api_key,
...
export async function POST(request: Request) {
  try {
    ...
    if (!isValidAiProvider(provider)) {
      return bad(`Unsupported provider "${provider}"`)
    }
    ...
    if (!Number.isFinite(maxPer)) maxPer = 20
    ...
    const existingDecoded = existing
      ? decodeDbModel(existing.provider, existing.model)
      : null
    ...
    const credentialsChanged =
      !existingDecoded ||
      rawKey !== '' ||
      provider !== existingDecoded.provider ||
      model !== existingDecoded.model ||
      baseUrl !== existingDecoded.baseUrl
    ...
    const encoded = encodeDbModel(provider, model, baseUrl)
    const encryptedKey = rawKey ? encrypt(rawKey) : null
    const shared: Record<string, unknown> = {
      provider: encoded.dbProvider,
      model: encoded.dbModel,
