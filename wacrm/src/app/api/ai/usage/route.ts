...
import { decodeDbModel } from '@/lib/ai/config'
...
export async function GET(request: Request) {
  try {
    ...
    for (const r of rows) {
      ...
      const decoded = decodeDbModel(r.provider, r.model)
      const mk = `${decoded.provider}:${decoded.model}`
      const m =
        modelMap.get(mk) ??
        { model: decoded.model, provider: decoded.provider, calls: 0, tokens: 0 }
      m.calls += 1
