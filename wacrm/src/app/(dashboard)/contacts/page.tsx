...
export default function ContactsPage() {
  ...
  // 'any' | 'has' | 'missing'
  const [emailFilter, setEmailFilter] = useState<'any' | 'has' | 'missing'>('any');
  ...
  const fetchContacts = useCallback(async () => {
    ...
    if (selectedTagIds.length > 0) {
      ...
      // Apply additional client-side filters on the tag-filtered result set
      let filtered = rows.map((r) => r.contact);
      ...
      if (emailFilter === 'has') filtered = filtered.filter((c) => !!c.email);
      if (emailFilter === 'missing') filtered = filtered.filter((c) => !c.email);
      contactRows = filtered;
    ...
    setLoading(false);
  }, [supabase, page, search, selectedTagIds, companyFilter, dateFrom, dateTo, emailFilter, tagsMap, t]);
  ...
  // Count of non-tag active filters (for badge on the Filter button)
  const extraFilterCount =
    (companyFilter.trim().length > 0 ? 1 : 0) +
    (dateFrom.length > 0 || dateTo.length > 0 ? 1 : 0) +
    (emailFilter !== 'any' ? 1 : 0);
  const totalFilterCount = selectedTagIds.length + extraFilterCount;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
              ...
              <div className="divide-y divide-border">
                  ...
                  {allTags.length === 0 ? (
                    ...
                    <div className="max-h-40 overflow-y-auto space-y-0.5">
                      {allTags.map((tag) => (
                        <label
                          key={tag.id}
                          className="flex items-center gap-2.5 py-1 cursor-pointer hover:bg-muted/50 rounded px-1"
                        >
                          <Checkbox
                ...
                <div className="px-3 py-2">
                  ...
                  <Input
                    ...
                    className="h-8 text-sm bg-card border-border text-foreground placeholder:text-muted-foreground"
                  />
                </div>
                ...
                <div className="px-3 py-2">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 h-8 text-xs rounded-md border border-border bg-card text-foreground px-2 focus:outline-none focus:ring-1 focus:ring-ring"
                      aria-label={t('filterDateFrom')}
                    />
                    <span className="text-xs text-muted-foreground shrink-0">{t('filterDateTo')}</span>
                    <input
                      type="date"
                      value={dateTo}
                ...
                {/* Email presence section */}
                <div className="px-3 py-2">
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-2">
                    {t('filterEmail')}
                  </p>
                  <div className="flex gap-1">
                    {(['any', 'has', 'missing'] as const).map((opt) => (
                      <button
                        key={opt}
                        onClick={() => { setEmailFilter(opt); setPage(0); }}
        ...
        {/* Active filter chips */}
        {(selectedTagIds.length > 0 || companyFilter.trim() || dateFrom || dateTo || emailFilter !== 'any') && (
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {companyFilter.trim() && (
              <span className="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-0.5 text-[11px] font-medium text-foreground">
                {t('filterCompany')}: {companyFilter.trim()}
                <button
                  onClick={() => { setCompanyFilter(''); setPage(0); }}
            ...
            {(dateFrom || dateTo) && (
              <span className="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-0.5 text-[11px] font-medium text-foreground">
                {dateFrom && dateTo
                  ? `${dateFrom} → ${dateTo}`
                  : dateFrom
                    ? `${t('filterDateFrom')} ${dateFrom}`
                    : `${t('filterDateTo')} ${dateTo}`}
                <button
            ...
            {emailFilter !== 'any' && (
              <span className="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-0.5 text-[11px] font-medium text-foreground">
                {t('filterEmail')}: {t(`filterEmail_${emailFilter}`)}
                <button
                  onClick={() => { setEmailFilter('any'); setPage(0); }}
