...
import { Loader2, Sparkles, CheckCircle2, Trash2, Eye, EyeOff, Globe } from 'lucide-react';
...
const PROVIDER_LABEL: Record<AiProvider, string> = {
  ...
  mistral: 'Mistral AI (Large / Small)',
  xai: 'xAI (Grok 2)',
  cohere: 'Cohere (Command R+)',
  custom: 'Custom (OpenAI-compatible / Ollama)',
};
...
const KEY_PLACEHOLDER: Record<AiProvider, string> = {
  ...
  xai: 'xai-...',
  cohere: '...',
  custom: 'sk-... (optional for local Ollama)',
};
...
export function AiConfig() {
  ...
  const handleSave = async () => {
    ...
    if (!configured && !keyEdited && provider !== 'custom') {
  ...
  const suggestedModels = AI_PROVIDER_SUGGESTED_MODELS[provider] ?? [];
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="cohere">{PROVIDER_LABEL.cohere}</SelectItem>
            ...
            {provider === 'custom' && (
              <div className="space-y-2">
                <Label htmlFor="ai-base-url" className="flex items-center gap-1.5">
                  <Globe className="h-3.5 w-3.5" /> Base URL (Endpoint)
                </Label>
                <Input
                  id="ai-base-url"
                  value={baseUrl}
                ...
                <p className="text-xs text-muted-foreground">
                  Works with Ollama, LM Studio, LocalAI, vLLM, Azure OpenAI, Together AI, etc.
                </p>
              </div>
            )}
