...
export function WhatsAppConfig() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (metaAppSecretEdited && metaAppSecret !== MASKED_TOKEN) {
        payload.meta_app_secret = metaAppSecret.trim();
      } else if (metaAppSecret === MASKED_TOKEN) {
      ...
      if (tokenEdited && accessToken !== MASKED_TOKEN && accessToken.trim()) {
        ...
        // Existing config — reuse stored encrypted token on server
          ...
          <CardContent className="space-y-4">
            ...
            <div className="pt-4 border-t border-border space-y-4">
              <div className="space-y-1">
                <h4 className="text-sm font-medium text-foreground">Meta App Developer Credentials</h4>
                <p className="text-xs text-muted-foreground">
                  Paste your Meta App ID and Meta App Secret from Meta for Developers (App Settings → Basic) to avoid setting server environment variables.
                </p>
              </div>

              <div className="space-y-2">
                <Label className="text-muted-foreground">Meta App ID <span className="text-xs text-muted-foreground">(Optional)</span></Label>
                <Input
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Required for resumable media upload when creating/editing image-header templates.
                </p>
              </div>

              <div className="space-y-2">
                <Label className="text-muted-foreground">Meta App Secret <span className="text-xs text-muted-foreground">(For Webhook Signature Verification)</span></Label>
                <div className="relative">
                  <Input
                  ...
                  <button
                    ...
                    className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors"
                  >
                    {showMetaAppSecret ? <EyeOff className="size-4" /> : <Eye className="size-4" />}
                  </button>
                </div>
                <p className="text-xs text-muted-foreground">
                  Stored AES-256-GCM encrypted in database. Used to verify HMAC signature on inbound webhook POSTs.
                </p>
              </div>
            </div>
          </CardContent>
